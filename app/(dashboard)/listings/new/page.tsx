...
import { ArrowLeft, X, Home, MapPin, Users, IndianRupee, Sparkles, ScrollText, ImagePlus, Plus } from 'lucide-react';
import Link from 'next/link';
import type { Amenity, PropertyType, RoomType, CancellationPolicy } from '@/types';
...
export default function CreateListingPage() {
  ...
  useEffect(() => {
    listingsApi.getAmenities().then((res) => setAmenities(res.data)).catch(() => {});
  }, []);
  ...
  const addRule = () => {
    ...
    if (trimmed && !houseRules.includes(trimmed)) {
  ...
  const onSubmit = async (data: CreateListingFormValues) => {
    try {
      ...
      if (data.cleaningFee && data.cleaningFee > 0) payload.cleaningFee = data.cleaningFee;
      if (data.minimumStay && data.minimumStay > 1) payload.minimumStay = data.minimumStay;
      if (selectedAmenities.length > 0) payload.amenityIds = selectedAmenities;
      if (houseRules.length > 0) payload.houseRules = houseRules.map((r) => ({ ruleText: r }));
      ...
      if (imageFiles.length > 0) {
        for (const file of imageFiles) {
          try {
            await listingsApi.addImage(listingId, file);
          } catch {
          ...
          <CardContent className="space-y-4 p-6">
            <div className="flex gap-2">
              <FloatingInput
                ...
                className="flex-1"
              />
              <Button type="button" variant="outline" className="h-[3.25rem] px-5" onClick={addRule}>Add</Button>
            </div>
            {houseRules.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {houseRules.map((rule, i) => (
                  <Badge key={i} variant="secondary" className="gap-1.5 py-1.5 pl-3 pr-2">
                    {rule}
                    <button type="button" onClick={() => removeRule(i)} className="rounded-full p-0.5 hover:bg-muted-foreground/20">
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
                ))}
        ...
        <Card className="overflow-hidden border-0 shadow-md">
          <div className="flex items-center gap-2.5 border-b bg-muted/30 px-6 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Add photos of your property (first image becomes cover)</p>
            </div>
          </div>
          <CardContent className="space-y-4 p-6">
            <label className="cursor-pointer">
              <input
                ...
                className="hidden"
              />
              <div className="flex items-center justify-center rounded-lg border-2 border-dashed border-muted-foreground/25 p-8 transition-colors hover:border-muted-foreground/50 hover:bg-muted/30">
              ...
              <div className="grid gap-3 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {imagePreviews.map((url, i) => (
                  <div key={i} className="relative group rounded-lg overflow-hidden border">
                    ...
                    {i === 0 && (
                      <span className="absolute top-2 left-2 inline-flex items-center rounded-full bg-primary px-2 py-0.5 text-[10px] font-medium text-primary-foreground">
                        Cover
                      </span>
                    )}
                    <button
